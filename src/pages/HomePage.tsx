...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-app pb-24 overflow-hidden">
      {/* Ambient glow */}
      ...
      <div className="relative px-6 pt-12 pb-2">
        <h1 className="text-[26px] font-bold text-fg">Hornstrike 🦄</h1>
        {current && (
          <div className="relative mt-1 inline-block">
            <button
              onClick={() => workspaces.length > 1 && setSwitchOpen(o => !o)}
              className="text-fg/55 text-sm flex items-center gap-1"
            >
              <span>{current.clubName} · {current.teamName}</span>
              {workspaces.length > 1 && <span className="text-fg/40">▾</span>}
            </button>
            {switchOpen && (
              <>
                <div className="fixed inset-0 z-40" onClick={() => setSwitchOpen(false)} />
                <div className="absolute left-0 top-7 z-50 bg-surface border border-fg/10 rounded-xl py-1 min-w-[220px] shadow-xl">
                  {workspaces.map(w => (
                    <button
                      key={w.teamId}
                      onClick={() => { setCurrentTeam(w.teamId); setSwitchOpen(false) }}
                      className={`w-full text-left px-3 py-2 text-sm ${w.teamId === currentTeamId ? 'text-accent-cyan font-semibold' : 'text-fg/80'}`}
                    >
      ...
      <div className="relative px-6 mt-4">
        <p className="text-[22px] font-bold text-fg">Bereit für den Spieltag?</p>
      ...
      <div className="relative flex gap-3 px-6 mt-6">
        <button
          ...
          className="flex-1 bg-surface rounded-2xl p-4 text-left"
        >
          <div className="w-11 h-11 rounded-full bg-unicorn-cyan/20 flex items-center justify-center text-2xl mb-4">👥</div>
          <p className="text-fg font-semibold text-lg">Spieler</p>
          <p className="text-fg/55 text-xs mt-0.5">{players.length} im Kader</p>
        ...
        >
          ...
          <p className="text-fg font-semibold text-lg">Spieltag</p>
          <p className="text-fg/55 text-xs mt-0.5">Aufstellung planen</p>
      ...
      <div className="relative px-6 mt-3">
        <button
          ...
          className="w-full bg-surface rounded-2xl p-4 flex items-center gap-3 text-left"
        >
          <div className="w-11 h-11 rounded-full bg-unicorn-gold/20 flex items-center justify-center text-2xl">📅</div>
          <div className="flex-1">
            <p className="text-fg font-semibold text-lg">Terminfindung</p>
            <p className="text-fg/55 text-xs mt-0.5">Verfügbarkeit fürs Team abfragen</p>
          </div>
      ...
      <div className="relative px-6 mt-8">
        <p className="text-fg/45 text-[13px] font-semibold tracking-wider uppercase mb-3">Letzter Spieltag</p>
        {lastMatch ? (
          <button
            onClick={() => navigate(`/lineup/${lastMatch.id}`)}
            className="w-full bg-surface rounded-2xl p-4 text-left"
          >
            <p className="text-fg font-semibold">{formatDate(lastMatch.date)}</p>
            <p className="text-accent-cyan text-sm mt-1">{lastMatch.players.length} Spieler · Aufstellung ansehen</p>
          </button>
        ) : (
          <div className="bg-surface rounded-2xl p-4">
            <p className="text-fg/40 text-sm">Noch kein Spieltag geplant</p>
          </div>
        )}
      </div>
      ...
      <div className="relative px-6 mt-4">
        <div className="bg-surface2 rounded-xl px-4 py-3">
          <p className="text-fg/65 text-[13px] leading-relaxed">
