  ...
  return (
    <div className="min-h-screen bg-unicorn-purple pb-32">
      ...
      <div className="relative px-6 space-y-3 mt-4">
        ...
        {/* Goalie */}
        <button onClick={() => setUseGoalie(v => !v)} className="bg-[#2b0b4c] rounded-2xl px-4 py-3.5 flex items-center justify-between w-full">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-white/45 text-[13px] font-semibold tracking-widest uppercase">Aktive Spieler ({activePlayers.length})</p>
            <button
              onClick={() => {
                ...
                <div key={player.id} className="bg-[#2b0b4c] rounded-2xl overflow-hidden">
                  <div className="flex items-center px-4 py-3.5 gap-3">
                    <button
                      ...
                      className={`w-6 h-6 rounded-lg flex-shrink-0 flex items-center justify-center transition-colors ${isChecked ? 'bg-unicorn-pink' : 'bg-[#391060] border border-white/20'}`}
                    >
                      {isChecked && <span className="text-white text-xs font-bold">✓</span>}
                    </button>
                    <span className={`flex-1 text-[15px] font-semibold transition-opacity ${isChecked ? 'text-white' : 'text-white/35'}`}>{player.name}</span>
                    {isChecked && (
                      <>
                        ...
                        <button onClick={() => setExpanded(isExpanded ? null : player.id)} className="text-white/30 text-sm ml-1" style={{ transform: isExpanded ? 'rotate(180deg)' : 'none' }}>▾</button>
                      </>
                    )}
                  </div>
                  {isExpanded && (
                    <div className="px-4 pb-4 pt-0 border-t border-white/5 space-y-3">
                      {(['from', 'to'] as const).map(key => (
                        <div key={key} className="flex items-center gap-3">
                          <span className="text-white/50 text-sm w-28">{key === 'from' ? 'Ab Spiel Nr.' : 'Bis Spiel Nr.'}</span>
                          <input
      ...
      <div className="fixed bottom-0 left-0 right-0 px-6 pb-8 pt-4 bg-gradient-to-t from-unicorn-purple via-unicorn-purple/95 to-transparent space-y-3">
