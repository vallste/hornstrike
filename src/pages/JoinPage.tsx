...
export default function JoinPage() {
  ...
  const inputCls = 'w-full rounded-2xl bg-white/8 border border-white/12 px-4 py-3.5 text-white placeholder-white/30 outline-none focus:border-unicorn-pink/60'
  ...
  return (
    <div className="relative w-full h-dvh min-h-screen overflow-hidden bg-unicorn-purple flex flex-col items-center justify-center px-6">
      ...
      >
        ...
        <h1 className="text-white font-black text-2xl text-center">Einladung zu Hornstrike</h1>
        <p className="text-unicorn-pink/80 text-center text-sm mt-1 mb-8 tracking-wide">
        ...
        {session ? (
          <div className="space-y-4">
            <p className="text-white/70 text-sm text-center">
              Eingeloggt als <span className="text-white font-medium break-all">{user?.email}</span>
            </p>
            <button onClick={acceptInvite} disabled={busy} className={ctaCls}>
              {busy ? 'Nehme an…' : 'Einladung annehmen'}
            </button>
            <button onClick={() => signOut()} className="w-full text-white/45 text-sm py-1">
              Nicht du? Abmelden
            </button>
          </div>
        ) : !sent ? (
          ...
          <div className="space-y-4">
            ...
            <button onClick={() => { setSent(false); setCode(''); setError(null) }} className="w-full text-white/45 text-sm py-1">
              ← andere E-Mail
            </button>
          </div>
        )}
