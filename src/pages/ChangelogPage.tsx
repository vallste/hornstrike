    ...
    <div className="min-h-screen bg-unicorn-purple pb-24">
      ...
      <div className="relative px-6 space-y-3 mt-2">
        {CHANGELOG.map((entry, idx) => {
          ...
          return (
            <div key={entry.version} className="bg-[#2b0b4c] rounded-2xl overflow-hidden">
              {/* Version header */}
              <button
              ...
              >
                <div className="flex-1 flex items-center gap-2.5 flex-wrap">
                  <span className="text-white font-bold text-[16px]">v{entry.version}</span>
                  {isLatest && (
                    <span className="text-[10px] font-bold bg-unicorn-cyan/15 text-unicorn-cyan px-2 py-0.5 rounded-full">
                      Aktuell
                    </span>
                  )}
                    ...
                    <div className="px-4 pb-4 pt-1 border-t border-white/5 space-y-2.5">
                      {entry.changes.map((c, i) => (
                        <div key={i} className="flex items-start gap-2.5">
                          ...
                          <span className="text-white/65 text-[13px] leading-snug">{c.text}</span>
                        </div>
                      ))}
                    </div>
                  </motion.div>
